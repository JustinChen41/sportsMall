		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				var html= '';
	            	...
	            	html += '				<div class="pic">';
	            	html += '					<input id="fileImage" type="file" size="100" name="imgFile[]" style="opacity:0;font-size:0;width:40px;height:40px;float:left;cursor:pointer" '+multiple+'>';
	            	html += '				</div>';
					html += '			</div>';
					html += '			<div id="preview" style="display:none" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div class="upload_submit" style="display:none">';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var delHtml = "";
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del" data-index="'+file.index+'" title="删除" onclick="deleteFile('+file.index+')"></span>';
				}
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '<li class="out">';
					html += '<div class="datetime">'+time_str+'</div>';
					html += '<img class="avatar" alt="" src="'+$("#userportrait").val()+'"/>';
					html += '<div class="message">';
					html += '<div class="pop">';
					html += '<span class="tail"></span>';
					html += '<img src="' + e.target.result + '" width="400px;"/>'; //图片路径
					html += '</div>';
					html += '</div>';
					html += '</li>';
				}else{
					...
					html += '<div class="pop">';
					html += '<span class="tail"></span>';
					html += '<img src="' + fileImgSrc + '" width="400px;"/>'; //图片路径
					html += '</div>';
					html += '</div>';
					html += '</li>';
				}
