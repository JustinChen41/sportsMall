...
var ZYFILE = {
		...
		url : "/Application/newsFileUpload",  					  // 上传action路径
		...
		funUploadFile : function(file){
			...
			formdata.append("kids", $('#kids').val());		
			formdata.append("fileIndexs", $('#fileIndexs').val());				
			...
			if(version.length > 1){
				...
				if((browser=="Microsoft Internet Explorer" && trim_Version=="MSIE8.0")){
					...
					// 完成
					xhr.attachEvent("onload", function(e){
						// 从文件中删除上传成功的文件  false是不执行onDelete回调方法
						self.funDeleteFile(file.index, false);
						// 回调到外部
						self.onSuccess(file, xhr.responseText);
						$('#kid').val(xhr.responseText);
						$('#kids').val(xhr.responseText);
						var newIndex = parseInt($("#fileIndexs").val())+1;
						$("#fileIndexs").val(newIndex);
					...
					// 错误
					xhr.attachEvent("onerror", function(e){
						// 回调到外部
		...
		init : function(){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if (this.dragDrop) {
				...
				if(version.length > 1){
					var trim_Version=version[1].replace(/[ ]/g,""); 
					if((browser=="Microsoft Internet Explorer" && trim_Version=="MSIE8.0")){
						this.dragDrop.attachEvent("ondragover", function(e) { self.funDragHover(e); }, false);
						this.dragDrop.attachEvent("ondragleave", function(e) { self.funDragHover(e); }, false);
						this.dragDrop.attachEvent("ondrop", function(e) { self.funGetFiles(e); }, false);
					} else {
			...
			if(self.uploadInput){
				...
				if(version.length > 1){
					var trim_Version=version[1].replace(/[ ]/g,""); 
					if((browser=="Microsoft Internet Explorer" && trim_Version=="MSIE8.0")){
						this.uploadInput.attachEvent("onclick", function(e) {
